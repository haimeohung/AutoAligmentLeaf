...
def cie_de_2000(lab1, lab2, k_L, k_C, k_H):
    ...
    G = 0.5*(1-np.sqrt(np.power(C_bar_star, 7) /
                       (np.power(C_bar_star, 7)+np.power(25, 7))))
    ...
    delta_H_dash = 2*np.sqrt(C_1_dash*C_2_dash) * \
        np.sin(np.radians(delta_h_dash)/2.0)
    ...
    T = 1-0.17*np.cos(np.radians(h_bar_dash-30))+0.24*np.cos(np.radians(2*h_bar_dash))\
        + 0.32*np.cos(np.radians(3*h_bar_dash+6))-0.20 * \
        np.cos(np.radians(4*h_bar_dash-63))

    delta_theta = 30 * np.exp(- np.power((h_bar_dash-275) / 25, 2))

    R_c = 2*np.sqrt(np.power(C_bar_dash, 7) /
                    (np.power(C_bar_dash, 7)+np.power(25, 7)))

    S_L = 1+((0.015*np.power(L_bar_dash-50, 2)) /
             np.sqrt(20+np.power(L_bar_dash-50, 2)))
...
def main():
    ...
    cie_de_2000([50.0000, 2.8361, -74.0200],
                [50.0000, 0.0000, -82.7485], 1, 1, 1)
